...
Public Class frmGame
    ...
    Private Const strGAME_VERSION As String = "1.61"
    ...
    Private Const intFOG_BACK_ADJUSTED_HEIGHT As Integer = intFOG_BACK_MEMORY_HEIGHT + (intORIGINAL_SCREEN_HEIGHT - (intFOG_BACK_Y + intFOG_BACK_MEMORY_HEIGHT)) 'This is the bottom cut off
    Private Const intFOG_FRONT_ADJUSTED_HEIGHT As Integer = intFOG_FRONT_MEMORY_HEIGHT + (intORIGINAL_SCREEN_HEIGHT - (intFOG_FRONT_Y + intFOG_FRONT_MEMORY_HEIGHT)) 'Bottom cut off
    ...
    Private pntVersusHostHover As New Point(115, 294)
    ...
    Private pntVersusJoinHover As New Point(459, 294)
    ...
    Private blnBackFromGame As Boolean
    ...
    Private intFaceZombieShake As Integer = 0
    ...
    Protected Overrides Sub WndProc(ByRef swfmSystemMessage As System.Windows.Forms.Message)
        ...
        If intResolutionMode = 5 Then
            ...
            If swfmSystemMessage.Msg = intWINDOW_MESSAGE_SYSTEM_COMMAND And swfmSystemMessage.WParam.ToInt32() = intCONTROL_MOVE Then
                'Exit
                Exit Sub
            End If
            'Prevent button down moving form
            If swfmSystemMessage.Msg = intWINDOW_MESSAGE_CLICK_BUTTON_DOWN And swfmSystemMessage.WParam.ToInt32() = intWINDOW_CAPTION Then
                'Exit
                Exit Sub
            End If
        End If
        ...
        If swfmSystemMessage.Msg = intWINDOW_MESSAGE_TITLE_BAR_DOUBLE_CLICKED Then
            'Exit
            Exit Sub
        End If

        'Still send message
        MyBase.WndProc(swfmSystemMessage) 'Must have mybase
    ...
    Private Sub SetImagesIntoMemory()
        ...
        LoadPictureFileAndCopyBitmapIntoMemory(btmVersusConnectTextHoverFile, btmVersusConnectTextHoverMemory, "Images\Versus\VersusConnectHover.png")
    ...
    Private Sub RemoveGameObjectsFromMemory(Optional blnStopScreamSound As Boolean = True)
        ...
        If udcOnScreenWordOne IsNot Nothing Then
            'Stop and dispose
            udcOnScreenWordOne.StopAndDispose()
            udcOnScreenWordOne = Nothing
        End If

        'Stop and dispose on screen word (Joiner)
        If udcOnScreenWordTwo IsNot Nothing Then
            'Stop and dispose
            udcOnScreenWordTwo.StopAndDispose()
            udcOnScreenWordTwo = Nothing
        End If
    ...
    Private Sub LoadHighscoresStringAccess(Optional blnRunErrorCode As Boolean = True)
        ...
        Dim dbaDataAdapter As System.Data.OleDb.OleDbDataAdapter

        'Prevent errors
        Try

            'Set
            dbaDataAdapter = New System.Data.OleDb.OleDbDataAdapter(strSQL, strConnection)

            'Set
            dbaDataAdapter.Fill(dtbDataTable)

            'Load string
            For intLoop As Integer = 0 To dtbDataTable.Rows.Count - 1
                'Add to string
                strHighscores &= dtbDataTable.Rows(intLoop).Item(0).ToString & ". " & dtbDataTable.Rows(intLoop).Item(1).ToString & strDRAW_STATS_SECONDS_WORD & " " &
                              dtbDataTable.Rows(intLoop).Item(2).ToString & " WPM " & dtbDataTable.Rows(intLoop).Item(3).ToString & " zombie kills - " &
                              dtbDataTable.Rows(intLoop).Item(4).ToString
                'Add new line if necessary
                If intLoop <> dtbDataTable.Rows.Count - 1 Then
    ...
    Private Sub LoadingGameScreen()
        ...
        'Text for controls
        DrawTextWithShadows("(Spacebar to reload), (' to run), (; to stop)", 20, Color.White, 185, 385, 500, 50)
        ...
        If blnBlackScreenFinished Then
            ...
            If Not blnPlayerWasPinned Then
                ...
                Select Case gintLevel
                    ...
                    Case 4
                        'Write to screen
                        DrawTextWithShadows("This level is not created yet and therefore you have become lost in the swamp and perished.", 25, Color.White, 50, 100,
                                            700, 100)
                    Case 5
                        'Show win overlay
    ...
    Private Sub EnterTheInformationIntoTheDatabaseAccess(strName As String, intRank As Integer)
        ...
        Dim strSQL As String = "SELECT * FROM HighscoresTable ORDER BY intRank ASC"
        Dim strConnection As String = "Provider = Microsoft.ACE.OLEDB.12.0;Data Source = Highscores.accdb"
        Dim dbcConnection As New System.Data.OleDb.OleDbConnection(strConnection)
        Dim dbaDataAdapter As New System.Data.OleDb.OleDbDataAdapter()

        'Set
        dbaDataAdapter.SelectCommand = New System.Data.OleDb.OleDbCommand(strSQL, dbcConnection) 'Necessary for updating data table after modification

        'More declares
        Dim dbbCommandBuilder As System.Data.OleDb.OleDbCommandBuilder = New System.Data.OleDb.OleDbCommandBuilder(dbaDataAdapter) 'Necessary read above
        Dim dtbDataTable As New DataTable
        ...
        If intRank <> 10 Then
            For intLoop As Integer = (dtbDataTable.Rows.Count - 1) To intRank Step -1
                dtbDataTable.Rows(intLoop).Item(1) = dtbDataTable.Rows(intLoop - 1).Item(1) 'Time
                dtbDataTable.Rows(intLoop).Item(2) = dtbDataTable.Rows(intLoop - 1).Item(2) 'WPM
                dtbDataTable.Rows(intLoop).Item(3) = dtbDataTable.Rows(intLoop - 1).Item(3) 'Zombie kills
                dtbDataTable.Rows(intLoop).Item(4) = dtbDataTable.Rows(intLoop - 1).Item(4) 'Name
            Next
        End If

        'Replace rank spot
        dtbDataTable.Rows(intRank - 1).Item(1) = CStr(CInt(tsTimeSpan.TotalSeconds))
        dtbDataTable.Rows(intRank - 1).Item(2) = CStr(intWPM)
        dtbDataTable.Rows(intRank - 1).Item(3) = CStr(intZombieKillsCombined)
        dtbDataTable.Rows(intRank - 1).Item(4) = strName

        'Update the database
        dbaDataAdapter.Update(dtbDataTable)

        'Close connection
        dbcConnection.Close()

        'Dispose
        dbcConnection.Dispose()
    ...
    Private Sub EnterTheInformationIntoTheDatabaseTextFile(strName As String, intRank As Integer)
        ...
        astrLine(intRank - 1) = CStr(CInt(tsTimeSpan.TotalSeconds)) & strDRAW_STATS_SECONDS_WORD & " " & intWPM & " WPM " & intZombieKillsCombined &
                                " zombie kills - " & strName
    ...
    Private Sub PlaySinglePlayerGame()
        ...
        Select Case gintLevel
            Case 1
                ...
                If gpntGameBackground.X < -450 Then
                    DrawGraphics(Graphics.FromImage(btmCanvas), btmDestroyedBrickWallMemory, gpntDestroyedBrickWall)
                End If
            Case 2
                'Chained zombie
                ...
                If gpntGameBackground.X <= -1550 And gpntGameBackground.X >= -3200 Then
                    ...
                    If gpntGameBackground.X <= -2550 Then
                        ...
                        If Not blnOpenedEyes Then
                            ...
                            'Set
                            intFaceZombieShake = 0
                            'Set
                            blnOpenedEyes = True
                        End If
                        'If eyes opened
                        If blnOpenedEyes Then
                            'Check which way to move
                            Select Case intFaceZombieShake
                                Case 0
                                    'Move right
                                    gudcFaceZombie.Point = New Point(gudcFaceZombie.Point.X + 4, gudcFaceZombie.Point.Y)
                                Case 2
                                    'Move left
                                    gudcFaceZombie.Point = New Point(gudcFaceZombie.Point.X - 4, gudcFaceZombie.Point.Y)
                                Case 4
                            ...
                            End Select
                            'Change face shake
                            If intFaceZombieShake = 8 Then
                                'Reset
                                intFaceZombieShake = 0
                            Else
                                'Increase
                                intFaceZombieShake += 1
                            End If
                        End If
                    End If
                    'Draw
        ...
        DrawWordBarWithWording(Color.Red)
    ...
    Private Sub CheckTheKeyPressBuffer(udcCharacterType As clsCharacter, audcZombiesType() As clsZombie, ByRef intByRefZombieKills As Integer)
        ...
        If strKeyPressBuffer <> "" Then
            ...
            'Declare to be split
            Dim astrTemp() As String = Split(strKeyPressBuffer, ".") 'Each elemt is used because delimiter came first instead of last
    ...
    Private Sub CheckTheKeyPressBufferX(udcCharacterType As clsCharacter, audcZombiesType() As clsZombie, ByRef intByRefZombieKills As Integer)
        ...
        If strKeyPressBuffer <> "" Then
            ...
            For intLoop As Integer = 0 To (astrTemp.GetUpperBound(0) - 1)
                ...
                'Check if typing matches
                If strEntireLengthOfWords.Substring(0, 1) = LCase(astrTemp(intLoop)) Then

                    'Change the word by one less letter
                    strEntireLengthOfWords = strEntireLengthOfWords.Substring(1)

                    'Check if word is done
                    If strEntireLengthOfWords.Substring(0, 1) = " " Then
    ...
    Private Sub DrawWordBarWithWording(colColor As Color)
        ...
        DrawText(Graphics.FromImage(btmCanvas), strEntireLengthOfWordsMinusLetters, 25, Color.Black, 265, 47, 500, 50) 'Shadow
        DrawText(Graphics.FromImage(btmCanvas), strEntireLengthOfWordsMinusLetters, 25, Color.Black, 264, 46, 500, 50) 'Shadow
        DrawText(Graphics.FromImage(btmCanvas), strEntireLengthOfWordsMinusLetters, 25, Color.White, 262, 45, 500, 50) 'White text

        'Word overlay
        DrawText(Graphics.FromImage(btmCanvas), strEntireLengthOfWordsMinusLetters.Substring(0, 1), 25, colColor, 262, 45, 500, 50) 'Overlay
    ...
    Private Sub DrawOnScreenWord(udcOnScreenWord As clsOnScreenWord)
        ...
        If udcOnScreenWord.OnScreen Then
            'Draw
            DrawGraphics(Graphics.FromImage(btmCanvas), udcOnScreenWord.Image, udcOnScreenWord.Point)
        End If
    ...
    Private Sub VersusScreen()
        ...
        Select Case intCanvasVersusShow
            Case 0 'Nickname
                'Draw host button
                If intCanvasShow = 2 Then
                    'Draw host button as hovered
                    DrawGraphics(Graphics.FromImage(btmCanvas), btmVersusHostTextHoverMemory, pntVersusHostHover)
                Else
                    'Draw host button
                ...
                'Draw join button
                If intCanvasShow = 3 Then
                    'Draw join button as hovered
                    DrawGraphics(Graphics.FromImage(btmCanvas), btmVersusJoinTextHoverMemory, pntVersusJoinHover)
                Else
                    'Draw join button
                ...
                DrawTextWithShadows(strNickName, 55, Color.White, 51, 175, 750, 137)
            Case 1 'Host
                'Draw hosting text
                DrawTextWithShadows("Hosting...", 36, Color.Red, 300, 225, 500, 75)
            Case 2 'Join
                'Draw IP address
                DrawGraphics(Graphics.FromImage(btmCanvas), btmVersusIPAddressTextMemory, pntVersusBlackOutline)
                'Draw IP address to type
                DrawTextWithShadows(strIPAddressConnect, 55, Color.White, 51, 175, 750, 137)
                'Draw connect button
                If intCanvasShow = 4 Then
                    'Draw connect button as hovered
                    DrawGraphics(Graphics.FromImage(btmCanvas), btmVersusConnectTextHoverMemory, pntVersusConnectHover)
                Else
                    'Draw connect button
            ...
            Case 3 'Connecting
                ...
                DrawTextWithShadows("Connecting...", 36, Color.Red, 250, 225, 500, 75)
        End Select

        'Draw ip address text
        DrawTextWithShadows(strIPAddress, 36, Color.Red, 7, 12, 500, 75)

        'Draw port forwarding
        DrawTextWithShadows("Router port forwarding: 10101", 25, Color.White, 187, 437, 600, 62)
    ...
    Private Sub PlayMultiplayerGame()
        ...
        If Not blnHost Then
            ...
            'Zombies two
            CheckMultiplayerZombieKillBuffer(strZombieKillBufferTwo, udcCharacterTwo, gaudcZombiesTwo, intZombieKillsTwo) 'Joiner on the join screen doesn't need to shoot
        End If
        ...
        If blnHost Then
            DrawWordBarWithWording(Color.Red)
        Else
            DrawWordBarWithWording(Color.Blue)
        End If
    ...
    Private Sub CheckCharacterMultiplayerStatus(udcCharacterType As clsCharacter, audcZombiesType() As clsZombie, ByRef intByRefZombieKills As Integer)
    ...
    Private Sub CheckMultiplayerZombieKillBuffer(ByRef strByRefZombieKillBufferType As String, udcCharacterType As clsCharacter, audcZombiesType() As clsZombie,
                                                 ByRef intByRefZombieKills As Integer, Optional blnShoot As Boolean = False)
    ...
    Private Function blnMouseInRegion(pntMouse As Point, intImageWidth As Integer, intImageHeight As Integer, pntStartingPoint As Point) As Boolean
        ...
        'Debug.Print("1=" & CStr(CInt(pntStartingPoint.X * gdblScreenWidthRatio)))
        'Debug.Print("2=" & CStr(CInt(pntStartingPoint.X + intImageWidth)))
        'Debug.Print("3=" & CStr(CInt(pntStartingPoint.Y * gdblScreenHeightRatio)))
        'Debug.Print("4=" & CStr(CInt(pntStartingPoint.Y + intImageHeight)))

        'Return
        If pntMouse.X >= CInt(pntStartingPoint.X * gdblScreenWidthRatio) AndAlso
        pntMouse.X <= CInt((pntStartingPoint.X + intImageWidth) * gdblScreenWidthRatio) AndAlso
        pntMouse.Y >= CInt(pntStartingPoint.Y * gdblScreenHeightRatio) AndAlso
        ...
        Select Case intCanvasVersusShow
            Case 0 'Default screen
                Select Case True
                    ...
                    Case blnMouseInRegion(pntMouse, 273, 88, pntVersusHost)
                        'Hover sound
                        HoverText(2, "VersusHost")

                    Case blnMouseInRegion(pntMouse, 221, 85, pntVersusJoin)
                        'Hover sound
        ...
        Select Case intMemoryLoadPosition
            ...
            Case 187 'Be careful changing this number, find = check if previously loaded to the end
                ...
                Else
                    ...
                    If blnHost Then
                        'Character one
                        udcCharacterOne = New clsCharacter(Me, intCHARACTER_HOSTER_X, intCHARACTER_HOSTER_Y, "udcCharacterOne", udcReloadingSound, udcGunShotSound, udcStepSound,
                                                           udcWaterFootStepLeftSound, udcWaterFootStepRightSound, udcGravelFootStepLeftSound,
                                                           udcGravelFootStepRightSound) 'Host
                        'Character two
                        udcCharacterTwo = New clsCharacter(Me, intCHARACTER_JOINER_X, intCHARACTER_JOINER_Y, "udcCharacterTwo", udcReloadingSound, udcGunShotSound, udcStepSound,
                                                           udcWaterFootStepLeftSound, udcWaterFootStepRightSound, udcGravelFootStepLeftSound,
    ...
    Private Sub LoadZombiesByType(ByRef audcByRefZombiesType() As clsZombie, intZombieX As Integer, intZombieY As Integer,
        ...
        For intLoop As Integer = 0 To (intNUMBER_OF_ZOMBIES_CREATED - 1)
            ...
            'Setup zombie
            audcByRefZombiesType(intLoop) = New clsZombie(Me, intZombieXToBe + intZombieBetweenZombieWidth, intZombieYToBe, 5 + intZombieSpeedIncrease,
                                            strThisObjectNameCorrespondingToCharacter, audcZombieDeathSounds, udcWaterSplashSound, blnAnimationStartRandom, blnImitation)
            'Increase
    ...
    Private Sub LoadingBitmapsIntoMemory()
        ...
        Select Case intMemoryLoadPosition
            ...
            Case 181
                'Memory load
                CopyBitmapIntoMemoryAfterDrawingScreen(btmDestroyedBrickWallFile, btmDestroyedBrickWallMemory, blnDestroyedBrickWallMemoryCopied)
            Case 182, 183
                'Memory load array
                CopyBitmapIntoMemoryAfterDrawingScreenWithIndex(182, abtmOnScreenWordMissedRedFiles, gabtmOnScreenWordMissedRedMemories,
                                                                ablnOnScreenWordMissedRedMemoriesCopied)
            Case 184, 185
                'Memory laod array
                CopyBitmapIntoMemoryAfterDrawingScreenWithIndex(184, abtmOnScreenWordMissedBlueFiles, gabtmOnScreenWordMissedBlueMemories,
                                                                ablnOnScreenWordMissedBlueMemoriesCopied)
    ...
    Private Sub CopyBitmapIntoMemoryAfterDrawingScreenWithIndex(intIndexToSubtract As Integer, ByRef abtmByRefFile() As Bitmap, ByRef abtmByRefMemory() As Bitmap,
                                                                ByRef ablnByRefMemoryCopied() As Boolean, Optional blnRotateFlip As Boolean = False,
    ...
    Private Sub SetLoadingBarPercentage(intMemoryLoadPositionToCheck As Integer, intPictureMemoryIndex As Integer)
        ...
        If intMemoryLoadPosition = intMemoryLoadPositionToCheck Then
            ...
            If btmLoadingBar IsNot abtmLoadingBarPictureMemories(intPictureMemoryIndex) Then
                'Set
